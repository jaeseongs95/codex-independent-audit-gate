...
2. **최종 대상 고정** — 요구사항과 함께 정확한 commit SHA, diff, artifact digest, 배포 ID 또는 관측 가능한 상태를 기록한다. 아직 최종 대상이 없는 설계 단계에서는 최종 판정을 내리지 않는다.
3. **자체 검증 수집** — 구현자가 실행한 테스트, 원시 결과, 알려진 한계, rollback 절차와 배포 전후 확인 항목을 모은다. 테스트 통과만으로 `PASS`를 추정하지 않는다.
4. **독립 감사 배정** — 구현자와 다른 fresh auditor에게 완결된 briefing과 원자료 접근 경로를 준다. 원하는 결론, 구현자의 자기평가, 다른 reviewer의 결론은 전달하지 않는다.
5. **직접 감사** — 감사자는 요구사항 충족, 최종 변경, 검증의 재현 가능성, 관련 실패 모드, 복구 가능성과 배포 상태를 직접 확인한다.
...
8. **배포 후 확인** — 프로덕션 또는 공개 릴리스는 실제 배포 식별자, 상태, smoke test와 rollback 필요성을 확인한다. 배포 전 `PASS`가 배포 후 상태까지 자동 보증하지 않는다.
...
- 배포 작업이라면 배포 전 감사와 배포 후 상태 확인을 구분해 기록했다.
