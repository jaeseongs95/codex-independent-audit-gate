...
특정 버전을 고정하려면 clone 명령에 `--branch v0.1.0 --depth 1`을 추가합니다.
...
나중에 여러 전문 스킬을 하나의 플러그인으로 배포할 때는 이 저장소의 `SKILL.md`, `agents/`, `references/`, `scripts/`를 플러그인의 `skills/independent-audit-gate/` 아래에 복사합니다. `README.md`, `LICENSE`, `.github/` 같은 저장소 메타데이터는 플러그인 루트 정책에 맞춰 별도로 관리합니다.

이 저장소는 `independent-deliberation-panel`을 개발 검토에 사용하지만 런타임 의존성으로 요구하지 않습니다.
